...
## Notes on editor warnings

Engine startup warnings like missing profiling DLLs or unrelated plugin icons are common in editor environments and are not GA execution failures.

## Source control guidance

Keep in Git:

- `EvoCompPlugin.uplugin`
- `Source/**`
- `Resources/**`
- `Config/**` (if used)
- `Content/**` (including `BP_GA_Main` and other assets)
- `.gitignore`
...
1. Build and validate in editor.
2. Commit source and content changes.
3. Tag a release (for example, `v0.1.0`).
4. Package plugin from Unreal Editor or via `BuildPlugin`.
5. Publish packaged zip and reuse in another project's `Plugins` folder.

## Build and package (CLI)

Use Unreal AutomationTool to validate and package this plugin:

1. Open a terminal.
2. Run:

  C:/EpicGames/UE_5.7/Engine/Build/BatchFiles/RunUAT.bat BuildPlugin -Plugin="C:/path/to/evocomp-ue5-plugin/EvoCompPlugin.uplugin" -Package="C:/path/to/output/EvoCompPlugin-packaged" -TargetPlatforms=Win64

3. Copy the packaged `EvoCompPlugin` folder into another project's `Plugins` directory.

## Migration compatibility

This plugin includes `CoreRedirects` so projects that previously used `GeneticAlgorithmPlugin` can load renamed classes and content paths under `EvoCompPlugin`.
